...
export default function IncomeSourceManager() {
  const { incomeSources, loading, create, update, remove } = useIncomeSources();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Cannot delete - source is in use");
      setDeleteId(null);
    }
  };
    ...
    <div>
      ...
      <ul className="space-y-2">
        {incomeSources.map((source) => (
          <li key={source.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            {editingId === source.id ? (
              <div className="flex items-center gap-2 flex-1">
                <input
                  ...
                  autoFocus
                />
                <button onClick={() => handleUpdate(source.id)} className="p-1 text-green-600 hover:bg-green-50 rounded">
                  <Check size={16} />
                </button>
      ...
      <ConfirmDialog
        ...
        message="Are you sure? This will fail if income entries are using this source."
      />
    </div>
  );
}
